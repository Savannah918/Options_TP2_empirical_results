...
Violation_rate_time_series.ipynb -- it plots the violation rates (both with and without tolerance), and number of 4-tuples over time, it also can save these data into csv files.
delete_files.ipynb -- it delete files with name match a certain pattern, it move files to another directory.
Submitting_job_function_parameter_changer.ipynb -- it change parameters of 10 files(each represent a parameter, 7-1(7 days before the expiration date, with transition length = 1), 7-3, 7-5, 7-7, 5-1, 5-3, 5-5, 3-1, 3-3, 1-1) when submitting a batch of jobs.
